...
const Mutex &IPT::getIptLock() const {
    return ipt_lock_;
}

void IPT::setIptLock(const Mutex &iptLock) {
    ipt_lock_ = iptLock;
}

const ustl::map<size_t, IPTEntry *> &IPT::getIpt() const {
    return ipt_;
}

void IPT::setIpt(const ustl::map<size_t, IPTEntry *> &ipt) {
    ipt_ = ipt;
}

const ustl::map<size_t, IPTEntry *> &IPT::getSipt() const {
    return sipt_;
}

void IPT::setSipt(const ustl::map<size_t, IPTEntry *> &sipt) {
    sipt_ = sipt;
}

size_t IPT::getDirtySwaps() const {
    return dirty_swaps_;
}

void IPT::setDirtySwaps(size_t dirtySwaps) {
    dirty_swaps_ = dirtySwaps;
}

size_t IPT::getCleanSwaps() const {
    return clean_swaps_;
}

void IPT::setCleanSwaps(size_t cleanSwaps) {
    clean_swaps_ = cleanSwaps;
}
