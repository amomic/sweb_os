...
void
ArchThreads::createUserRegisters(ArchThreadRegisters *&info, void *start_function, void *user_stack, void *kernel_stack)
{
...
void ArchThreads::printThreadRegisters(Thread *thread, size_t userspace_registers, bool verbose)
{
    ...
    if (!info)
    {
        kprintfd("%sThread: %18p, has no %s registers. %s\n", userspace_registers ? "  User" : "Kernel", thread,
                 ...
                 userspace_registers ? "" : "This should never(!) occur. How did you do that?");
    } else if (verbose)
    {
        kprintfd("\t\t%sThread: %18p, info: %18p\n"\
             ...
             "\t\t\trflg: %18zx  cr3: %18zx\n",
                 userspace_registers ? "  User" : "Kernel", thread, info, info->rax, info->rbx, info->rcx, info->rdx,
                 info->rsp, info->rbp, info->rsp0, info->rip, info->rflags, info->cr3);
    } else
    {
        kprintfd(
                "%sThread: %18p, info: %18p -- rax: %18zx  rbx: %18zx  rcx: %18zx  rdx: %18zx -- rsp: %18zx  rbp: %18zx  rsp0 %18zx -- rip: %18zx  rflg: %18zx  cr3: %zx\n",
                userspace_registers ? "  User" : "Kernel", thread, info, info->rax, info->rbx, info->rcx, info->rdx,
                info->rsp, info->rbp, info->rsp0, info->rip, info->rflags, info->cr3);
    }
}
...
void ArchThreads::debugCheckNewThread(Thread *thread)
{
    ...
    assert(thread->kernel_registers_ != 0 && thread->kernel_registers_ != currentThread->kernel_registers_ &&
    ...
    assert(thread->kernel_registers_->rsp != currentThread->kernel_registers_->rsp &&
    ...
    if (currentThread->user_registers_ == 0)
        return;
    assert(currentThread->user_registers_->rsp0 != thread->user_registers_->rsp0 &&
           "no 2 threads may have the same esp0 value");
}

uint64 ArchThreads::rdtsc()
{
    unsigned long lo, hi;
